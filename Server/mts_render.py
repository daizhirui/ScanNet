...
def render(args):
    ...
    mesh = pmgr.create(
        {"type": "ply", "filename": meshfile, "bsdf": {"type": "diffuse", "reflectance": {"type": "vertexcolors"}}}
    )
    scene = Scene()
...
def main():
    ...
    parser.add_argument("--cameras", dest="cameras", action="store", help="Cameras file to render")
    parser.add_argument(
        "--integrator",
        dest="integrator",
        action="store",
    ...
    parser.add_argument(
        "--samples", dest="samples", action="store", default=16, type=int, help="Number of integrator samples per pixel"
    )
    parser.add_argument("--width", dest="width", action="store", default=640, type=int, help="Image width")
    parser.add_argument("--height", dest="height", action="store", default=480, type=int, help="Image height")
    parser.add_argument(
        "--frames_per_degree", dest="frames_per_degree", action="store", default=5, type=int, help="Image height"
    )
    parser.add_argument(
        "--theta", dest="theta", action="store", default=0, type=float, help="Azimuth angle to render from"
    )
    parser.add_argument(
        "--exposure",
        dest="exposure",
    ...
    parser.add_argument(
        "--world_up", dest="world_up", action="store", default=Vector(0, 0, 1), type=nvec3, help="World up vector"
    )
    parser.add_argument(
        "--camera_up",
        dest="camera_up",
