...
VOXELHASHING_BIN = os.path.join(
    VOXELHASHING_DIR, "DepthSensing.exe zParametersScanNet.txt zParametersTrackingDefault.txt"
)
...
def process_scan_dir_basic(path, name, config):
    ...
    if config.get("overwrite") or not os.path.isfile(sensfile):
        ...
        if config.get("calibrate"):
            ...
            ret = util.call(
                [CALIBRATE_BIN, uncalibrated_sensfile, sensfile, DEVICES_CSV, DEVICES_DIR],
                log,
                CALIBRATE_DIR,
                desc="calibrate",
            )
    ...
    if config.get("clean"):
        ...
        ret = util.call(
            cfg.MESHLAB_BIN + " -i " + plyfile + " -o " + plyfile + " -m vc -s " + MESHPROC_DIR + "cleanLoRes.mlx",
            log,
            desc="clean1",
        )
    ...
    if config.get("thumbnail"):
        ret = util.call(
            ...
            log,
            env=dict(os.environ, CONVERT=IMG_CONVERT_BIN, PNGQUANT=PNGQUANT_BIN, OPTIPNG=OPTIPNG_BIN),
            desc="thumbnail",
        )
...
def main():
    ...
    parser.add_argument(
        "-b", "--batch", dest="batch", action="store_true", default=False, help="Batch processing of input directory"
    )
    parser.add_argument(
        "--from", dest="from", action="store", default="convert", choices=PROCESSES, help="Which command to start from"
    )
    parser.add_argument("--action", dest="actions", action="append", choices=PROCESSES, help="What actions to do")
    parser.add_argument(
        "--overwrite", dest="overwrite", action="store_true", default=False, help="Overwrite existing files"
    )
    parser.add_argument("--novh", dest="novh", action="store_true", default=False, help="Remove _vh suffixes")
    parser.add_argument(
        "--test", dest="test", action="store_true", default=False, help="Test pipeline commands without executing them"
    )
