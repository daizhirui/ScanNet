...
def receive_file(request, filename, output=None):
    ...
    while True:
        ...
        try:
            ...
            log.exception("Receiving %s: Exception: %s while reading input. Aborting...", filename, str(e))
    ...
    if content_read != content_length:
        log.error(
            "Receiving %s: Expected length %d, received length %d. Aborting...", filename, content_length, content_read
        )
...
def verify_file():
    ...
    if valid:
        ...
        log.error("File %s: hash mismatch. Given: %s, calculated: %s", filename, checksum, calculated_checksum)
        raise Error(
            message=("File hash mismatch. Given: %s, calculated: %s" % (checksum, calculated_checksum)), status_code=400
        )
