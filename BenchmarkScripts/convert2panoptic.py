...
def convert2panoptic(scannetPath, outputFolder=None):
    ...
    if not files:
        print("Did not find any files for using matching pattern {}. Please consult the README.".format(search))
        sys.exit(-1)
    # a bit verbose
    ...
    for idx in range(len(EVAL_LABELS)):
        ...
        isthing = label > 2
        categories.append({"id": int(label), "name": name, "color": color, "supercategory": cat, "isthing": isthing})
...
def main():
    ...
    parser.add_argument(
        "--output-folder", dest="outputFolder", help="path to the output folder.", default=None, type=str
    )
